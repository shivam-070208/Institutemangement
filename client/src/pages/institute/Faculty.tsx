...
function Faculty() {
  ...
  const fetchFaculty = async (page: number) => {
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/institute/fetchFaculty?page=${page}&limit=5`, {
        credentials: "include",
      });
      const data = await res.json();

      const faculty = data.faculty?.map((item: any) => ({
        ...item,
        id: item.faculty_id,
  ...
  const handleAdd = async () => {
    if (!newFaculty.name || !newFaculty.subjects || !newFaculty.contact_email || !newFaculty.password) {
    ...
    try {
      ...
      if (!response.ok) return toast(data.message || "Failed to add faculty ⚠️");

      const faculty = { ...data.faculty};
      setFacultyList((prev) => [faculty, ...prev]);
      setShowModal(false);
      setNewFaculty({ name: "", subjects: "", contact_email: "", password: "" });
    ...
    <div className="space-y-6 relative px-2 w-full flex flex-col flex-wrap">
      <div className="flex w-full justify-between flex-wrap flex-col md:flex-row">
        <div>
          ...
          <p className="text-muted-foreground">Manage faculty members and their subjects</p>
        </div>
        <div
      ...
      <div className="max-w-full">
        <DataTable data={facultyList as any} columns={columns} searchPlaceholder="Search faculty..." />
      </div>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  onChange={(e) => setNewFaculty({ ...newFaculty, contact_email: e.target.value })}
                />
                <Input
                  type="password"
